...
const SearchableItems = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://192.168.1.81:5002/api/foods'); // Replace with your API endpoint
      ...
      const formattedData = data.map((item) => ({
        ...
        type: item.type_food,
        imageUrl: `http://192.168.1.81:5002/api/${item.images[0]}`, // Dynamic image URL from API
        distance: '2.0',
        deliveryTime: '20-30',
        favorite: false,
        isOpen: true,
